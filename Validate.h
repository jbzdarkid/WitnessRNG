...
private:
  ...
  RegionData RegionCheckNegations2(Puzzle& puzzle, const Region& region, const std::vector<Cell*>& negationSymbols, const std::vector<Cell*>& invalidElements, int index = 0, int index2 = 0) {
    return RegionData();
  }
  ...
  RegionData RegionCheck(Puzzle& puzzle, const Region& region, bool quick = false);

  bool PolyFit(const Region& region, Puzzle& puzzle) {
    return true;
  }
};